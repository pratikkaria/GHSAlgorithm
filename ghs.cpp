  ...
  for(int j=0;j<allNodes.size();j++)
  {
    ...
    *logFile<<"First\t"<<"Second\t"<<"Weight\t"<<"State\t"<<"\n";
    for (auto i : allNodes[j]->adjacentEdges)
    {
      ...
      *logFile<<allNodes[j]->nodeId<<"\t"<<destNodeId<<"\t"<<edgeTo->weight<<"\t"<<edgeTo->state<<"\n";
    }
...
void logSummary(double executionTime, int numEdges, int numNodes)
{
  ofstream logFile;
  logFile.open ("summary.txt");
  //printAdjacentEdges(&logFile);
  logFile<<"Total Number of Nodes:"<<numNodes<<"\n";
  logFile<<"Total Number of Edges:"<<numEdges<<"\n";
  logFile<<"Execution Time of the program: "<< executionTime<<fixed<< setprecision(9)<<" seconds\n";
  printAdjacentEdges(&logFile);
  int totalMessages = 0;
  for(int i=0;i<allNodes.size();i++)
    totalMessages+=allNodes[i]->numberOfMessages;
  logFile<<"Total Messages : "<<totalMessages<<"\n";
  for(int i=0;i<allNodes.size();i++)
    logFile<<"Number of Messages Sent by Node:"<<i<<" is "<<allNodes[i]->numberOfMessages<<"\n";
...
int main(int argc,char** argv)
{
  ...
  if (file.is_open())
  {
      ...
      if(logEnables==1 || all==1)
        logSummary(time_taken,numEdges,numNodes);
      file.close();
  }
  else
  {
